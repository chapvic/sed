...
static void check_punctuation P_((FMT_WORD* w));
static COST base_cost P_((FMT_WORD* this));
static COST line_cost P_((FMT_WORD* next, int len));
static void put_paragraph P_((FMT_WORD* finish));
static void put_line P_((FMT_WORD* w, int indent));
static void put_word P_((FMT_WORD* w));
...
/* The words of a paragraph -- longer paragraphs are handled neatly
   (cf. flush_paragraph()).  */
static FMT_WORD words[MAXFMT_WORDS];
...
{
    int start;
    register FMT_WORD* end_of_word;

    end_of_word = &words[MAXFMT_WORDS - 2];
...
static void
check_punctuation(register FMT_WORD* w)
{
...
flush_paragraph(void)
{
    ...
    memmove((char*)words, (char*)split_point,
        (word_limit - split_point + 1) * sizeof(FMT_WORD));
...
static COST
base_cost(register FMT_WORD* this)
{
    register COST cost;
...
static COST
line_cost(register FMT_WORD* next, register int len)
{
    register int n;
    register COST cost;
...
static void
put_paragraph(register FMT_WORD* finish)
{
    register FMT_WORD* w;
...
static void
put_line(register FMT_WORD* w, int indent)
{
    register FMT_WORD* endline;
    out_column = 0;
    put_space(indent);
